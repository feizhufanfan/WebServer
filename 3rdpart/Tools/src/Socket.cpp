...
void Socket::setOnRead(onReadCB cb) {
    ...
    if (cb) {
        ...
        _on_read = [](const Buffer::Ptr &buf, struct sockaddr *, int) {
            LOG_WARN(logger,"Socket not set read callback, data ignored:%d",buf->size());
        };
    }
}
...
void Socket::setOnErr(onErrCB cb) {
    ...
    if (cb) {
        ...
        _on_err = [](const SockException &err) {
            LOG_WARN(logger,"Socket not set err callback, err:%s", err.what());
        };
    }
}
...
void Socket::setOnAccept(onAcceptCB cb) {
    ...
    if (cb) {
        ...
        _on_accept = [](Socket::Ptr &sock, shared_ptr<void> &complete) {
            LOG_WARN(logger,"Socket not set accept callback, peer fd:%d",sock->rawFD());
        };
    }
}
    ...
    while (_enable_recv) {
        ...
        if (nread == 0) {
            if (!is_udp) {
                ...
                LOG_WARN(logger,"recv eof on udp socket[%d]",sock_fd);
            }
            return ret;
        }
        ...
        if (nread == -1) {
            ...
            if (err != UV_EAGAIN) {
                if (!is_udp) {
                    ...
                    LOG_WARN(logger,"recv err on udp socket[%d]:%s" ,sock_fd, uv_strerror(err));
                }
            }
            return ret;
        }
        ...
        try {
            ...
            LOG_ERR(logger,"触发socket on_read事件时,捕获到异常:%s",ex.what());
        }
    }
    return 0;
}
...
bool Socket::emitErr(const SockException& err) noexcept{
    ...
    _poller->async([weak_self, err]() {
        ...
        try {
            ...
            LOG_ERR(logger,"触发socket on_err事件时,捕获到异常:%s",ex.what());
        }
    });
    ...
    while (true) {
        if (event & EventPoller::Event_Read) {
            ...
            if (fd == -1) {
                ...
                emitErr(ex);
                LOG_ERR(logger,"tcp服务器监听异常:%s", ex.what());
                return -1;
            }
            ...
            try {
                ...
                LOG_ERR(logger, "触发socket before accept事件时,捕获到异常:%s" , ex.what());
                close(fd);
                continue;
            }
            ...
            try {
                ...
                LOG_ERR(logger,"触发socket accept事件时,捕获到异常:%s",ex.what());
                continue;
            }
        }
...
bool Socket::flushData(const SockFD::Ptr &sock, bool poller_thread) {
    ...
    while (!send_buf_sending_tmp.empty()) {
        ...
        if (is_udp) {
            // udp发送异常，把数据丢弃
            send_buf_sending_tmp.pop_front();
            LOG_WARN(logger,"send udp socket[%d] failed, data ignored:%s",fd,uv_strerror(err));
            continue;
        }
        // tcp发送失败时，触发异常
        emitErr(toSockException(err));
...
bool Socket::cloneFromListenSocket(const Socket &other){
    SockFD::Ptr sock;
    {
        ...
        if (!other._sock_fd) {
            LOG_WARN(logger,"sockfd of src socket is null!");
            return false;
        }
...
bool Socket::bindPeerAddr(const struct sockaddr *dst_addr, socklen_t addr_len) {
    ...
    if (-1 == ::connect(_sock_fd->rawFd(), dst_addr, addr_len ? addr_len : SockUtil::get_sock_len(dst_addr))) {
        LOG_WARN(logger,"connect peer address failed:%s",SockUtil::inet_ntoa(dst_addr).c_str());
        return false;
    }
    return true;
}
