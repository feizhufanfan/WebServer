...
EventPoller::~EventPoller() {
    ...
    _loop_thread_id = this_thread::get_id();
    onPipeEvent();
    LOG_INFOCount(logger)<< this;
}

int EventPoller::addEvent(int fd, int event, PollEventCB cb) {
   //TimeTicker();
    if (!cb) {
        LOG_WARN(logger, "PollEventCB 为空!");
        return -1;
    }
...
inline void EventPoller::onPipeEvent() {
    ...
    _list_swap.for_each([&](const Task::Ptr &task) {
        try {
            ...
            LOG_WARNCout(logger) << "EventPoller执行异步任务捕获到异常:" << ex.what();
        }
    });
}
        ...
        while (!_exit_flag) {
            ...
            for (int i = 0; i < ret; ++i) {
                ...
                try {
                    ...
                    LOG_ERRCout(logger) << "EventPoller执行事件回调捕获到异常:" << ex.what();
                }
            }
        }
#else
        ...
        while (!_exit_flag) {
            ...
            callback_list.for_each([](Poll_Record::Ptr &record) {
                try {
                    ...
                    LOG_ERR(logger, "EventPoller执行事件回调捕获到异常:%s", ex.what());
                }
            });
            callback_list.clear();
        }
...
uint64_t EventPoller::flushDelayTask(uint64_t now_time) {
    ...
    for (auto it = task_copy.begin(); it != task_copy.end() && it->first <= now_time; it = task_copy.erase(it)) {
        ...
        try {
            ...
            LOG_ERRCout(logger)<< "EventPoller执行延时任务捕获到异常:" << ex.what();
        }
    }
...
EventPollerPool::EventPollerPool() {
    ...
    LOG_INFO(logger, "创建EventPoller个数:%d" , size);
}
