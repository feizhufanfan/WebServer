...
UdpServer::~UdpServer() {
    if (!_cloned && _socket->rawFD() != -1) {
        LOG_INFO(logger, "close udp server [%s]:", _socket->get_local_ip().c_str() , _socket->get_local_port());
    }
    _timer.reset();
    _socket.reset();
...
void UdpServer::start_l(uint16_t port, const std::string &host) {
    ...
    LOG_INFO(logger,"UDP Server bind to [%s]:%d",host.c_str(),port);
}
...
void UdpServer::onRead_l(bool is_server_fd, const UdpServer::PeerIdType &id, const Buffer::Ptr &buf, sockaddr *addr, int addr_len) {
    ...
    if (auto session = getOrCreateSession(id, buf, addr, addr_len, is_new)) {
        if (session->getPoller()->isCurrentThread()) {
            ...
            //数据漂移到其他线程，需要先切换线程
            LOG_WARN(logger,"udp packet incoming from other thread");
...
#if !defined(NDEBUG)
        if (!is_new) {
            LOG_TraceL(logger) << "udp packet incoming from " << (is_server_fd ? "server fd" : "other peer fd");
        }
#endif
    }
}
