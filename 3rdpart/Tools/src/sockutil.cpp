...
int SockUtil::setCloseWait(int fd, int second) {
    ...
    if (ret == -1) {
#ifndef _WIN32
        LOG_TraceL(logger)<< "设置 SO_LINGER 失败!";
#endif
    }
    return ret;
}
...
int SockUtil::setNoDelay(int fd, bool on) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 NoDelay 失败!";
    }
    return ret;
}
...
int SockUtil::setReuseable(int fd, bool on, bool reuse_port) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 SO_REUSEADDR 失败!";
        return ret;
    }
    ...
    if (reuse_port) {
        ...
        if (ret == -1) {
             LOG_TraceL(logger) << "设置 SO_REUSEPORT 失败!";
        }
    }
#endif
    return ret;
...
int SockUtil::setBroadcast(int fd, bool on) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 SO_BROADCAST 失败!";
    }
    return ret;
}
...
int SockUtil::setKeepAlive(int fd, bool on) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 SO_KEEPALIVE 失败!";
    }
    return ret;
}
...
int SockUtil::setCloExec(int fd, bool on) {
    ...
    if (flags == -1) {
         LOG_TraceL(logger) << "设置 FD_CLOEXEC 失败!";
        return -1;
    }
    if (on) {
        flags |= FD_CLOEXEC;
...
int SockUtil::setNoSigpipe(int fd) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 SO_NOSIGPIPE 失败!";
    }
    return ret;
#else
    return -1;
...
private:
    ...
    std::shared_ptr<struct addrinfo> getSystemDomainIP(const char *host) {
        ...
        if (!answer) {
             LOG_WARNCout(logger) << "域名解析失败:" << host;
            return nullptr;
        }
...
static int set_ipv6_only(int fd, bool flag) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IPV6_V6ONLY 失败!";
    }
    return ret;
}
...
static int bind_sock6(int fd, const char *ifr_ip, uint16_t port) {
    ...
    if (1 != inet_pton(AF_INET6, ifr_ip, &(addr.sin6_addr))) {
        if (strcmp(ifr_ip, "0.0.0.0")) {
             LOG_WARNCout(logger) << "inet_pton to ipv6 address failed:" << ifr_ip;
        }
        addr.sin6_addr = IN6ADDR_ANY_INIT;
    }
    if (::bind(fd, (struct sockaddr *) &addr, sizeof(addr)) == -1) {
         LOG_WARNCout(logger) << "绑定套接字失败:" << get_uv_errmsg(true);
        return -1;
    }
    return 0;
}
...
static int bind_sock4(int fd, const char *ifr_ip, uint16_t port) {
    ...
    if (1 != inet_pton(AF_INET, ifr_ip, &(addr.sin_addr))) {
        if (strcmp(ifr_ip, "::")) {
             LOG_WARNCout(logger) << "inet_pton to ipv4 address failed:" << ifr_ip;
        }
...
int SockUtil::connect(const char *host, uint16_t port, bool async, const char *local_ip, uint16_t local_port) {
    ...
    if (sockfd < 0) {
         LOG_WARNCout(logger) << "创建套接字失败:" << host;
        return -1;
    }
    ...
    if (async && get_uv_error(true) == UV_EAGAIN) {
     ...
     LOG_WARNCout(logger) << "连接主机失败:" << host << " " << port << " " << get_uv_errmsg(true);
    close(sockfd);
    return -1;
}
...
int SockUtil::listen(const uint16_t port, const char *local_ip, int back_log) {
    ...
    if ((fd = (int)socket(family, SOCK_STREAM, IPPROTO_TCP)) == -1) {
         LOG_WARNCout(logger) << "创建套接字失败:" << get_uv_errmsg(true);
        return -1;
    }
    ...
    if (::listen(fd, back_log) == -1) {
         LOG_WARNCout(logger) << "开始监听失败:" << get_uv_errmsg(true);
        close(fd);
        return -1;
    }
...
void for_each_netAdapter_posix(FUN &&fun){ //type: struct ifreq *
    ...
    if (-1 == ioctl(sockfd, SIOCGIFCONF, &ifconf)) {    //获取所有接口信息
         LOG_WARNCout(logger) << "ioctl 失败:" << get_uv_errmsg(true);
        close(sockfd);
        return;
    }
    close(sockfd);
...
int SockUtil::dissolveUdpSock(int fd) {
    ...
    if (-1 == ::connect(fd, (struct sockaddr *)&addr, addr_len) && get_uv_error() != UV_EAFNOSUPPORT) {
        // mac/ios时返回EAFNOSUPPORT错误
         LOG_WARNCout(logger) << "解除 UDP 套接字连接关系失败: " << get_uv_errmsg(true);
        return -1;
    }
   return 0;
}
...
string SockUtil::get_ifr_mask(const char *if_name) {
    ...
    if ((ioctl(fd, SIOCGIFNETMASK, &ifr_mask)) < 0) {
         LOG_WARNCout(logger) << "ioctl 失败:" << if_name << " " << get_uv_errmsg(true);
        close(fd);
        return "";
    }
    close(fd);
...
int SockUtil::setMultiTTL(int fd, uint8_t ttl) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_MULTICAST_TTL 失败!";
    }
#endif
...
int SockUtil::setMultiIF(int fd, const char *local_ip) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_MULTICAST_IF 失败!";
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::setMultiLOOP(int fd, bool accept) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_MULTICAST_LOOP 失败!";
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::joinMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_ADD_MEMBERSHIP 失败:" << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::leaveMultiAddr(int fd, const char *addr, const char *local_ip) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_DROP_MEMBERSHIP 失败:" << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::joinMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_ADD_SOURCE_MEMBERSHIP 失败:" << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
...
int SockUtil::leaveMultiAddrFilter(int fd, const char *addr, const char *src_ip, const char *local_ip) {
    ...
    if (ret == -1) {
         LOG_TraceL(logger) << "设置 IP_DROP_SOURCE_MEMBERSHIP 失败:" << get_uv_errmsg(true);
    }
#endif
    clearMulticastAllSocketOption(fd);
    return ret;
