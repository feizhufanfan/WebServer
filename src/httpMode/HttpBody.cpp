...
static std::shared_ptr<char> getSharedMmap(const string &file_path, int64_t &file_size) {
    ...
    if (fd < 0) {
        LOG_WARN(logger,  "fileno failed:%s",get_uv_errmsg(false));
        return nullptr;
    }
    auto ptr = (char *)mmap(NULL, file_size, PROT_READ, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
        LOG_WARN(logger,  "mmap:%s failed:%s",file_path.c_str(),get_uv_errmsg(false));
...
Buffer::Ptr HttpFileBody::readData(size_t size) {
    ...
    if (!_map_addr) {
        ...
        LOG_WARN(logger,"read file err:%s", get_uv_errmsg());
        return nullptr;
    }
