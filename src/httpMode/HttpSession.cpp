...
ssize_t HttpSession::onRecvHeader(const char *header,size_t len) {
    ...
    if (it == s_func_map.end()) {
        LOG_WARN(logger,"不支持该命令: %s(%s:%d) %s" ,this->getIdentifier().c_str(),this->get_peer_ip().c_str(),this->get_peer_port(),cmd.c_str());
...
void HttpSession::onError(const SockException& err) {
    //http客户端
    LOG_ERR(logger,"%s(%s:%d)  %s" ,this->getIdentifier().c_str(),this->get_peer_ip().c_str(),this->get_peer_port(), err.what());
}
